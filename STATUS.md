...
## Solution and Technology
   The further into the development we got, the more we became comvfortable with the overall layout and structure we planned during the beginning stages. Small tweaks were made to some base classes to help with things such as enemy hitbox detection, level scrolling, and player movement. We feel that classes such as the handler class, which ticks and renders any game object, have been extremely helpful in our development to allow one layer of abstraction when implementing things such as the lazer shooting and enemy AI. This structure, that was layed out in the original proposal, will continue to be useful moving forward, especially as we added more enemies and features.  
   ...
   One great part of developing a game is that the testing is rather painless. While we may sometimes be disappointed our implementations fail, testing has been rather enjoyable. That being said, we have tested our game extensivley, trying anything possible to find possible bugs. This has resulted in quite a few changes to our implementations for player movement and creating a side-scrolling level. 
...
**Reflection**
  ...
  The only things that have not gone smoothly involve the more complex implementation of level scrolling and detection of the player near the edge of the screen. While we still have a working base game, bugs are still being fixed regarding those issues. 
  There won't be much done differently during the next iteration, if we can continue the progress made then we will hopefully have a game all the team members are proud to have worked on. 
